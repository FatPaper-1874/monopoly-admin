...
onMounted(() => {
  ...
  const app = new PIXI.Application({ view: pixiCanvas, width: props.width, height: props.height, backgroundColor: 0xFFCF70 });
  ...
  watch(currentMapItemId, (newVal) => {
    ...
    if (mapItem) {
      ...
      lineCell.drawRect(mapItem.x * cellSize, mapItem.y * cellSize, cellSize * mapItem.type.size, cellSize * mapItem.type.size);
      lineCell.endFill();
      app.render();
    } else {
      lineCell.clear();
...
<template>
  <div class="map-editor-page">
    ...
    <div class="canvas-container">
      <div class="current-coordinate">当前坐标: ({{ currentX }}, {{ currentY }}) 当前类型: {{ currentType?.name }} 当前模型: {{
        currentType?.module }}</div>
    ...
    <div class="tool-container">
              ...
              <el-select value-key="color" clearable v-model="currentType" placeholder="选择类型进入添加模式">
                <el-option v-for="item, index in typeList" :key="item.color" :value="item" :label="item.name">
                  <div class="flex align-center justify-space-between">
                    ...
                    <span @click.stop="handleRemoveType(index)"
                      style="color: var(--el-color-danger); font-size: 0.8; float: right;">删除</span>
                  </div>
                </el-option>
              </el-select>
            </el-form-item>
      ...
      <div class="map-item-editor">
        <div class="current-item">
          ...
          <el-input clearable style="margin-bottom: 5px;" v-model="linkTargetId" placeholder="输入连接目标id">
            <template #append>
              <el-button :disabled="(currentMapItemId === '')" @click="handleLink(currentMapItemId, linkTargetId)"
                type="primary">link</el-button>
            </template>
          </el-input>
          <el-button style="width: 100%;" :disabled="(currentMapItemId === '')" type="danger"
            @click="handleRemoveMapItem(currentMapItemId)">delete</el-button>
        </div>
      ...
      <div class="map-data-export">
        ...
        <el-button :disabled="mapData.length == 0 || mapName == ''"
          @click="handleExportMapData()" type="primary">导出</el-button>
      </div>
