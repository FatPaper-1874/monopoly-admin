...
import {bindArrivedEventToMapItem, getArrivedEventList, unbindArrivedEventFromMapItem} from "@/utils/api/arrivedEvent";
  ...
  <div class="arrived-event-selector">
    <div class="current-arrived-event-info">
      <div class="info-container" v-if="currentArrivedEvent">
        <div style="font-size: .9em; font-weight: bold;" class="title">当前MapItem的到达事件：</div>
        <el-popover
        ...
        >
          <template #default>
            <div style="font-size: .9em; font-weight: bold; color: #000000">事件描述</div>
            <span>{{ currentArrivedEvent.describe }}</span>
          </template>
          <template #reference>
            <div class="info">
              <img style="width: 1.3em;height: 1.3em;" :src="`http://${currentArrivedEvent.iconUrl}`" alt="">
      ...
      <div style="font-size: .9em; font-weight: bold;" v-else>
        该MapItem没有绑定到达事件喔
      </div>
    </div>
    <div class="arrived-event-selector-form">
      <el-select class="selector" clearable v-model="selectedArrivedEventId" placeholder="选择到达事件"
                 style="width: 240px">
        <el-option
        ...
        >
          ...
          >
            ...
            <template #reference>
              <div class="options">
                <img style="width: 1.3em;height: 1.3em;" :src="`http://${arrivedEvent.iconUrl}`" alt="">
      ...
      <el-button :disabled="!selectedArrivedEventId" style="margin-left: 5px" type="primary" @click="handleBind">绑定
      </el-button>
    </div>
  </div>
</template>
